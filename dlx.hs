...
instance Eq (Node s) where
    (==) node1 node2 = idNode node1 == idNode node2
...
initColumns root nd 0 secondary = return []
initColumns root nd m secondary = do
    node <- newNode (-1) m

    if not $ secondary!!( length secondary - m ) then do
...
initDancingLinks matrix secondary = do
    ...
    root <- newNode (-1) (-1)
    columns <- initColumns root root m secondary
